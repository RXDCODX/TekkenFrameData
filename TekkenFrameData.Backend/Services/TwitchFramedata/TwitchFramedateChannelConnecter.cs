using tekkenfd.Services.Framedata;
...
public class TwitchFramedateChannelConnecter : IHostedService
{
    ...
    public async Task ConnectToStreams()
    {
        ...
        var newStreams = streams.Where(e => !joined.Any(joined => joined.Channel.Equals(e.UserLogin, StringComparison.OrdinalIgnoreCase)) && !e.Id.Equals("40792090693", StringComparison.OrdinalIgnoreCase));
        var streamsToLeave = joined.Where(e => !streams.Any(stream => stream.UserLogin.Equals(e.Channel, StringComparison.OrdinalIgnoreCase)));
        ...
        foreach (var stream in newStreams)
        {
            ...
            if (channelInformations is { FollowerMode: false, EmoteMode: false, SubscriberMode: false, UniqueChatMode: false })
            {
    ...
    private async Task<bool> ValidateToken(string token)
    {
        ...
        catch (Exception e) when (e.Message.Contains("invalid access token", StringComparison.OrdinalIgnoreCase))
        {
            return false;
        }
        catch (Exception e)
    ...
    private void FrameDateMessage(object? sender, OnMessageReceivedArgs args)
    {
        ...
        if (message.StartsWith("!fd ", StringComparison.OrdinalIgnoreCase))
        {
            ...
            if (split.Length > 2)
            {
                ...
                if (move != null)
                {
                    ...
                    message = "✅ " + move.Character.Name + " > " + move.Command + " ✅  " + "Block: " + move.BlockFrame
                              + " | Dmg: " + move.Damage
                              + " | Hit: " + move.HitFrame
                              + " | HitLvl: " + move.HitLevel
                              + " | StartUp: " + move.StartUpFrame
                              + (string.IsNullOrEmpty(teges) ? "" : " | Tags: " + teges);

                    try
                    {
                        if (!_client.JoinedChannels.Any(e => e.Channel.Equals(channel, StringComparison.OrdinalIgnoreCase)))
                        {
