...
public class FrameDataClient
{
    ...
    public async Task<TekkenCharacter?> GetCharacterAsync(int characterId)
    {
        try
        {
            var response = await _client.GetCharacterAsync(
        ...
        catch (Exception ex)
        {
            _logger.LogError(
                ex,
                "Error getting character {CharacterId} from Core service",
                characterId
            );
            return null;
        }
    ...
    public async Task<TekkenCharacter?> GetCharacterAsync(string characterName)
    {
        ...
        {
            var response = await _client.GetCharacterAsync(
                new GetCharacterRequest { CharacterName = characterName }
            );
            return response.Character;
        }
        catch (Exception ex)
        {
            _logger.LogError(
                ex,
                "Error getting character {CharacterName} from Core service",
                characterName
            );
            return null;
        }
    ...
    public async Task<IEnumerable<TekkenMove>> GetCharacterMovesAsync(int characterId)
    {
        try
        {
            var response = await _client.GetCharacterMovesAsync(
                new GetCharacterMovesRequest { CharacterId = characterId }
            );
        ...
        catch (Exception ex)
        {
            _logger.LogError(
                ex,
                "Error getting moves for character {CharacterId} from Core service",
                characterId
            );
    ...
    public async Task<IEnumerable<TekkenMove>> GetCharacterMovesAsync(string characterName)
    {
        ...
        {
            var response = await _client.GetCharacterMovesAsync(
                new GetCharacterMovesRequest { CharacterName = characterName }
            );
            return response.Moves;
        }
        catch (Exception ex)
        {
            _logger.LogError(
                ex,
                "Error getting moves for character {CharacterName} from Core service",
                characterName
            );
            return Enumerable.Empty<TekkenMove>();
        }
    ...
    public async Task<TekkenMove?> GetMoveAsync(int moveId)
    {
        try
        {
            var response = await _client.GetMoveAsync(new GetMoveRequest { MoveId = moveId });
    ...
    public async Task<TekkenMove?> GetMoveAsync(string moveName, int characterId = 0)
    {
        try
        {
            var response = await _client.GetMoveAsync(
                new GetMoveRequest { MoveName = moveName, CharacterId = characterId }
            );
            return response.Move;
        }
        catch (Exception ex)
    ...
    public async Task<IEnumerable<TekkenMove>> SearchMovesAsync(
        string query,
        int characterId = 0,
        int limit = 10
    )
    {
        try
        {
            var response = await _client.SearchMovesAsync(
                new SearchMovesRequest
                {
                    Query = query,
        ...
        catch (Exception ex)
        {
            _logger.LogError(
                ex,
                "Error searching moves with query {Query} from Core service",
                query
            );
            return Enumerable.Empty<TekkenMove>();
        }
