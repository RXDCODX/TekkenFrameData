...
docker build -f "..\TekkenFrameData.Backend\TekkenFrameData.Watcher\Dockerfile" -t tekken_frame_data_watcher:dev --no-cache ".."

:: Сборка образа Streamer
echo.
echo [2/4] Сборка образа TekkenFrameData.Streamer...
docker build -f "..\TekkenFrameData.Backend\TekkenFrameData.Streamer\TekkenFrameData.Streamer.Server\Dockerfile" -t tekken_frame_data.streamer:dev --no-cache ".."
